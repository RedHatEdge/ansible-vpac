...
4. **Network config** — sets `ceph config set global public_network <ceph.public_network>` and `cluster_network <ceph.cluster_network>` authoritatively (cephadm's bootstrap flags are unsupported / hint-only in current versions). Also binds the dashboard to `ceph_bootstrap_dashboard_server_addr` (default `0.0.0.0`) so the operator's mgmt-VLAN laptop can reach it — default binds only to the storage IP. Restarts MGR daemons so the dashboard rebinds.
