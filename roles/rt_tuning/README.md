...
1. **Install RT package set** — `kernel-rt`, `kernel-rt-kvm`, `tuned-profiles-realtime`, `tuned-profiles-nfv-host`, `realtime-tests`, `dnf-plugins-core`, `python3-dnf-plugin-versionlock`, `intel-cmt-cat`. Promote kernel-rt to default boot via `grubby --set-default-index <rt-index>`. Versionlock the stock `kernel`/`kernel-core`/`kernel-modules`; explicitly UN-versionlock `kernel-rt` family.
...
7. **cpufreq governor** — `cpupower frequency-set -g performance` live, plus `vpac-cpufreq.service` systemd oneshot to re-apply on boot.
8. **Verify** — `uname -r` ends `+rt`, tuned profile is `realtime-virtual-host`, isolated cpus match, `sched_rt_runtime_us` matches, `HugePages_Total` matches. (Most checks only hold after the reboot the role's notify scheduled.)
