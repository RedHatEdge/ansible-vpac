...
Reads from `group_vars/all/main.yml`: `deployment_mode`, `sources.*`, `rhsm_*`, `vpac_nodes`, `site_domain`, `site_timezone`.
...
If you are on a **legacy non-SCA account**, set `rhsm_pool` (in `group_vars/all/main.yml`) to a pool ID that entitles the repositories this deployment needs. Coverage across RHEL, the Add-Ons, and Red Hat Ceph Storage depends on the subscriptions your account holds — check with your Red Hat account team. The role passes it to `community.general.redhat_subscription` and attach happens at register time. Without `rhsm_pool` on a non-SCA account, the host registers but no repos are entitled and the next dnf step fails.
