...
1. **Preflight** — asserts the agent is supported (`fence_ipmilan` or `fence_virsh`), the agent binary is on PATH (provided by `fence-agents-all` from `pacemaker_base`), `corosync.conf` is present (pacemaker_base ran), and (lab path) `stonith.virsh_*` inventory vars are populated.
2. **Create fence devices** — branches on `stonith.fence_agent`:
   ...
   - **`fence_virsh`** (lab): copies the operator-supplied SSH identity to every cluster node as `0640 root:hacluster` (so the resource — running as the hacluster user — can read it), then creates one device per node with `pcmk_host_check=static-list`, `pcmk_host_list=<hostname>`, `pcmk_host_map=<hostname>:<libvirt_domain>` (the libvirt domain name often differs from the cluster hostname; default to hostname when `vpac_nodes[*].libvirt_domain` is unset).
3. **Constraints** — for every fence device, `pcs constraint location fence-<hostname> avoids <hostname>=INFINITY`. Idempotent: gates on the constraint listing so re-runs don't append duplicates with auto-generated IDs.
4. **Probe** *(optional)* — when `stonith_dry_run_actual_probe: true`, runs `fence_<agent> -o status` against each device. Proves the agent can reach the BMC (or libvirt host) without rebooting anything. Default off because some BMCs rate-limit query connections; turn on for first-deploy validation.
5. **Atomic enable** — reads `stonith_admin --list-registered`, asserts every expected `fence-<hostname>` is present, then `pcs property set stonith-enabled=true`. The atomicity matters: `stonith-enabled=true` with missing fence devices blocks all VirtualDomain resources cluster-wide because Pacemaker refuses to start a resource that requires fencing if it can't verify the fence path.
6. **Verify** — `pcs stonith status` reports no Stopped device, `stonith-enabled` confirmed `true`, no pending fencing actions in `pcs status`.
