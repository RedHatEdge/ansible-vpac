...
Then open **<http://127.0.0.1:8765>** in your browser. It is a local web page — nothing to install, nothing sent to the internet — that asks plain questions: where is this site, what are the node addresses, which disks for storage.

For the SSH key it lists the usable keys already in your `~/.ssh` so you can pick one, or creates a new dedicated key under a name you choose, and tests that the key and passwordless `sudo` actually work on every node.

Once that test passes, the form can **read the per-node answers off the nodes themselves** rather than asking you to type them: it fills in each node's network interfaces, and lists its disks with size and model for you to tick the ones to use as storage. **The disk holding the operating system is identified and excluded**, shown with the reason, and nothing is ticked for you — you choose, from a list you can check. This needs the nodes installed and reachable; on the air-gapped path, where you build the installer media before the nodes exist, these fields are filled in by hand and each one tells you the command that finds it.

The network addressing is different in kind and the form will not guess it: those are **design decisions** from your site's network plan, which QUICKSTART's "What you need" list tells you to prepare first.

When you press the button at the end, it writes your **complete site configuration, including the encrypted password vault**, into `inventory/<your-site>/`, and refuses with a plain-language list of problems if anything is wrong. That configuration is the single input everything else reads.
...
Building a **single node by hand** (no Ansible), with the ABB SSC600SW IED as the reference workload — useful for understanding what the playbooks do under the hood, or for a manual proof-of-concept:
