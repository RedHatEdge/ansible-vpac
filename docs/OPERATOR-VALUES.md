...
| `rhsm_activation_key`, `rhsm_org_id` | connected / satellite | strings | the node must be able to install from BaseOS, AppStream, CodeReady Builder, NFV (`kernel-rt`), High Availability, and Red Hat Ceph Storage tools. The Add-Ons and Ceph are separate products from RHEL — check with your Red Hat account team that your subscriptions cover all of them, for labs as well as production; | your RHSM or Satellite admin; keep credentials OUT of the inventory file (vault or extra-vars file) |
...
| `validate.*` thresholds | **before stage 90** — set from your ACTUAL hardware | `storage_nic_min_mbps` (e.g. `1000` on a 1 Gb storage network — the shipped `10000` fails stage 90 on any 1 Gb site even though 1 Gb is an acknowledged legitimate compromise), `cyclictest_max_latency_us` per your platform; the FULL threshold list (PTP hard-fail offset, iostat ceilings, clock skew, stonith requirement, …) is contract section 14, every key commented | your site's link speeds + measured RT latency; a threshold nobody told you about is a stage-90 surprise, so set these when you fill the file, not when validate fails |
| `container_images` | prefilled per RHCS release | image:tag map (ceph + monitoring) | change only when bumping the Ceph release or mirroring to a local registry — values track cephadm's own defaults, don't invent tags |
