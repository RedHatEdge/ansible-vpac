...
| `rhsm_activation_key`, `rhsm_org_id` | connected / satellite | strings | the subscription must cover **RHEL 9 + High Availability + Resilient Storage + NFV + Red Hat Ceph Storage** (ask for that list by name; RHEL alone fails mid-deploy); labs: the free Red Hat Developer subscription qualifies; | your RHSM or Satellite admin; keep credentials OUT of the inventory file (vault or extra-vars file) |
