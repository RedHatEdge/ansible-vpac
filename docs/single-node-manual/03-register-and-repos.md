...
1. **Red Hat account and subscription.** The host must be able to install from the RT/NFV repository (the source of `kernel-rt`), which comes from a RHEL Add-On rather than base RHEL — confirm your subscriptions cover it with your Red Hat account team. Accounts and subscriptions are managed at [access.redhat.com](https://access.redhat.com). Registration concepts and methods are described in [Getting Started with RHEL System Registration](https://docs.redhat.com/en/documentation/subscription_central/1-latest/html-single/getting_started_with_rhel_system_registration/index).
